...
function img_with_id($path, $width, $style, $id = "", $class = "") {
  ...
  echo '<img loading="lazy" src="', $path, '"'
     ...
     . (!empty($id) ? " id=\"{$id}\"" : '')
     . ' style="width:', $width, '%;', (!$as_aspect ? 'height:auto;' : ''), $style, '"/>';
}

function picture_with_id($path, $width, $style, $id = "", $class = "") {
  echo '<picture><source srcset="', pathinfo($path, PATHINFO_FILENAME), '.webp" type="image/webp"/>';
...
function generate_rss() {
  $rss_file   = "rss.xml";
  $rss_script = "rss.php";
  if (!file_exists($rss_script)) return;
  ob_start();
  include($rss_script);
  $rss = ob_get_clean();
...
function generate($src_dir) {
  ...
  if (file_exists($dst)) {
    ...
    if ($src_date < $dst_date && $header_date < $dst_date && $footer_date < $dst_date) {
      return;
    }
  }
